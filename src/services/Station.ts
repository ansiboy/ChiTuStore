...
class SiteService {
    searchProducts = (searchText: string, pageIndex: number = 0): JQueryPromise<any> => {
        ...
        return services.get(services.config.siteServiceUrl, 'Home/SearchProduct', data);
    }
    hotKeywords = (): JQueryPromise<string[]> => {
        return services.get(services.config.siteServiceUrl, 'Home/GetSearchKeywords');
    }
    historyKeyword = (): JQueryPromise<string[]>=> {
        return services.get(services.config.siteServiceUrl, 'Home/GetHistoryKeywords');
    }
}
