...
class ProductPanel {
    ...
    constructor(page: chitu.Page, model) {
        ...
        page.container.element.appendChild(this.node);
        //==========================================
        this._page.closed.add(() => {
            this.page_closed();
        });

        requirejs(['text!modules/Home/Product/ProductPanel.html'], this.html_loaded);
    }
    ...
    private html_loaded = (html: string) => {
        ...
        var iscroll = new IScroll($wrapper[0], { tap: true, preventDefault: true });
