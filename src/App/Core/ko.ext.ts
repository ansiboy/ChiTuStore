...
function tryUpdateScrollView(element: HTMLElement) {
    if (element == null) new Error('Argument element is null.');

    var $scroll_view = $(element).parents('scroll-view');
    if ($scroll_view.length > 0) {
        var scroll_view = <any>$scroll_view.data('control');
        if (scroll_view instanceof chitu.IScrollView) {
            refreshScrollView(<chitu.IScrollView>scroll_view);
        }
    }
}

function refreshScrollView(scroll_view: chitu.IScrollView) {
    var timeid = <any>$(scroll_view.element).data('timeid');
    if (timeid != null) {
